export default class RenderNode {
  ...
  animate(frame, fps) {
    this.animations.forEach((animation) => {
      ...
      let progress = 0;
      if (!animation.type || animation.type == "" || animation.type === "linear") {
  ...
  render(ctx) {
    ...
    ctx.fillRect(
      this.parent ? this.renderProps.x + this.parent.renderProps.x : this.renderProps.x,
      this.parent ? this.renderProps.y + this.parent.renderProps.y : this.renderProps.y,
      this.renderProps.width,
      this.renderProps.height
    );
    this.children.forEach((child) => {
      child.render(ctx);
    });
